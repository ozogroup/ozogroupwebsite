-- ============================================================================
-- MIGRATION: Membership Approval Fix V2 — 2026-07-22
-- ============================================================================
-- Removes the redundant RPC-level auth check that was causing
-- "Admin access required" errors even when called via the service-role client.
--
-- The JS layer already enforces admin access via requireAdmin() before
-- calling this RPC, and the service-role client bypasses RLS — so the
-- RPC-level auth check is unnecessary and was failing due to GUC variable
-- mismatch with PostgREST service-role JWTs.
--
...
BEGIN
  -- Auth check removed: requireAdmin() in JS + service-role client handle access control.
...
SELECT 'migration_membership_approval_fix_v2_2026_07_22_ready' AS status;
